...
__global__ void Gmove_trn(const InfoStruct *Ginfo, AtomArray *GfragmentInfo, Residue *GresidueInfo, 
    ...
    // Update atom positions after translation
    for (int i = tid; i < SharedFragmentInfo.num_atoms; i += numThreadsPerBlock) {
...
bool move_trn(const InfoStruct *infoHost, InfoStruct *infoDevice, AtomArray *fragmentInfoHost, AtomArray *fragmentInfoDevice, 
    ...
    Gmove_trn<<<numBlocks, numThreadsPerBlock>>>(infoDevice, fragmentInfoDevice, residueInfoDevice, atomInfoDevice, 
    ...
    while (confIndexUnused.size() > 0) {
        ...
        for (auto iit = confIndexUsed.begin(); iit != confIndexUsed.end(); ++iit) {
    ...
    if (needUpdate) {
        cudaMemcpy(tempInfoDevice, tempInfoHost, sizeof(Atom)*numBlocks, cudaMemcpyHostToDevice);
        GupdateTrn<<<numBlocks, numThreadsPerBlock>>>(fragmentInfoDevice, residueInfoDevice, atomInfoDevice, 
