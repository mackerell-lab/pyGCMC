...
class TestProject:
    ...
    # def test_print_methods(self, project, test_files, capsys):
    #     """Test various print methods for structure and force field information."""
    #     # Load structure with topology
    #     structure = project.load_structure(test_files['test.pdb'], test_files['test.top'])
    ...
    #     # Verify structure contains expected number of atoms
    #     assert len(structure.atoms) == 196, "Structure should contain 196 atoms"

    #     # Test print_nonbonded_params
    #     forcefield.print_nonbonded_params()
        
    #     # Capture the output using capsys
    #     captured = capsys.readouterr()
    #     nonbonded_out = captured.out

    #     # Print the output for debugging (optional)
    #     print("\nCaptured nonbonded output:")
    #     print(nonbonded_out)
        
    #     # Debug print each line
    #     print("\nDebug: Printing each line:")
    #     for i, line in enumerate(nonbonded_out.split('\n')):
    #         print(f"Line {i}: '{line}'")

    #     # Verify nonbonded parameters output format - case insensitive check
    #     has_type = any("type" in line.lower() for line in nonbonded_out.split('\n'))
    #     has_epsilon = any("epsilon" in line.lower() for line in nonbonded_out.split('\n'))
    #     has_rmin = any("rmin" in line.lower() for line in nonbonded_out.split('\n'))
        
    #     print(f"\nDebug: Column presence check results:")
    #     print(f"Has 'Type' column: {has_type}")
    #     print(f"Has 'Epsilon' column: {has_epsilon}")
    #     print(f"Has 'Rmin' column: {has_rmin}")
        
    #     assert has_type, "Output should contain 'Type' column"
    #     assert has_epsilon, "Output should contain 'Epsilon' column"
    #     assert has_rmin, "Output should contain 'Rmin' column"

    #     # Test print_all_atoms
    #     structure.print_all_atoms()
    #     captured = capsys.readouterr()
    #     atoms_out = captured.out

    #     # Print the output for debugging
    #     print("\nCaptured atoms output:")
    #     print(atoms_out)

    #     # Verify atoms output format
    #     assert any("Residue" in line for line in atoms_out.split('\n')), "Output should contain 'Residue' column"
    #     assert any("Seq" in line for line in atoms_out.split('\n')), "Output should contain 'Seq' column"
    #     assert any("Name" in line for line in atoms_out.split('\n')), "Output should contain 'Name' column"
    #     assert any("Type" in line for line in atoms_out.split('\n')), "Output should contain 'Type' column"
    #     assert any("TopoType" in line for line in atoms_out.split('\n')), "Output should contain 'TopoType' column"
    #     assert any("Charge" in line for line in atoms_out.split('\n')), "Output should contain 'Charge' column"
    #     assert any("Mass" in line for line in atoms_out.split('\n')), "Output should contain 'Mass' column"
    #     assert any("Epsilon" in line for line in atoms_out.split('\n')), "Output should contain 'Epsilon' column"
    #     assert any("Rmin" in line for line in atoms_out.split('\n')), "Output should contain 'Rmin' column"

    #     # Test print_forcefield_info
    #     forcefield.print_forcefield_info()
    #     captured = capsys.readouterr()
    #     ff_info_out = captured.out

    #     # Print the output for debugging
    #     print("\nCaptured forcefield info output:")
    #     print(ff_info_out)

    #     # Verify force field info output
    #     assert "Global Nonbonded Parameters:" in ff_info_out, "Missing global parameters section"
        
    #     # Extract and verify numerical values with tolerance
    #     for line in ff_info_out.split('\n'):
    #         if "cutoff:" in line:
    #             value = float(line.split(':')[1].strip())
    #             assert abs(value - 14.000) < 1e-3, "Incorrect cutoff value"
    #         elif "switching:" in line:
    #             value = float(line.split(':')[1].strip())
    #             assert abs(value - 12.000) < 1e-3, "Incorrect switching value"
    #         elif "pairlist_distance:" in line:
    #             value = float(line.split(':')[1].strip())
    #             assert abs(value - 16.000) < 1e-3, "Incorrect pairlist_distance value"
        
    #     # Verify NBFIX parameters in the output
    #     assert "NBFIX Parameters:" in ff_info_out, "Missing NBFIX parameters section"
    #     test_cases = [
    #         ("SOD", "CLA", -0.084, 3.731),
    #         ("BRGR1", "NC2", -1.100, 3.660),
    #         ("BRGR1", "HGP3", -0.240, 2.970),
    #         ("NC2", "OC", -0.155, 3.637),
    #         ("CLA", "POT", -0.114, 4.081)
    #     ]
    #     for type1, type2, eps, rmin in test_cases:
    #         found = False
    #         for line in ff_info_out.split('\n'):
    #             if type1 in line and type2 in line:
    #                 # Extract numerical values
    #                 values = [float(val) for val in line.split() if val.replace('-', '').replace('.', '').isdigit()]
    #                 if len(values) >= 2:
    #                     actual_eps = values[0]
    #                     actual_rmin = values[1]
    #                     assert abs(actual_eps - eps) < 1e-3, f"Incorrect epsilon for {type1}-{type2}"
    #                     assert abs(actual_rmin - rmin) < 1e-3, f"Incorrect rmin for {type1}-{type2}"
    #                     found = True
    #                     break
    #         assert found, f"NBFIX parameters for {type1}-{type2} not found in output" 