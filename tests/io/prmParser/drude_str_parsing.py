...
class TestDrudeSTRParsing:
    
    @pytest.fixture
    def str_file_path(self):
        """返回Drude STR文件路径"""
        return os.path.join(
    ...
    def test_parse_drude_str_file(self, str_file_path):
        """测试是否能解析Drude STR文件"""
    ...
    def test_bond_parameters_from_str(self, str_file_path):
        """测试键参数的读取"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_angle_parameters_from_str(self, str_file_path):
        """测试角参数的读取"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_dihedral_parameters_from_str(self, str_file_path):
        """测试二面角参数的读取"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_improper_parameters_from_str(self, str_file_path):
        """测试不当二面角参数的读取"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_nbfix_parameters_from_str(self, str_file_path):
        """测试NBFIX参数的读取"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_drude_alpha_thole_parameters(self, str_file_path):
    ...
    def test_lonepair_definitions(self, str_file_path):
        """测试LONEPAIR定义的读取
        这个测试可能会失败，因为标准PRM parser可能不支持
        """
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_anisotropy_definitions(self, str_file_path):
        """测试ANISOTROPY定义的读取
        这个测试可能会失败，因为标准PRM parser可能不支持
        """
        ff = pygcmc.PRMParser.parse_file(str_file_path)
    ...
    def test_atom_type_count(self, str_file_path):
        """测试原子类型数量"""
        ff = pygcmc.PRMParser.parse_file(str_file_path)
