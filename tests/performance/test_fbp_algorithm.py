...
def test_algorithm_comparison():
    ...
    for name in ["SCF", "OPT3", "Smart OPT3", "FBP"]:
        ax1.plot(water_sizes, results[name]['times'], 'o-', label=name)
    ax1.set_xlabel('Number of Waters')
    ax1.set_ylabel('Time (s)')
    ax1.set_title('Calculation Time vs System Size')
    ax1.legend()
    ax1.grid(True)
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ...
    # Speedup vs SCF
    scf_times = np.array(results['SCF']['times'])
    for name in ["OPT3", "Smart OPT3", "FBP"]:
        speedup = scf_times / np.array(results[name]['times'])
        ax2.plot(water_sizes, speedup, 'o-', label=name)
    ax2.set_xlabel('Number of Waters')
    ax2.set_ylabel('Speedup vs SCF')
    ax2.set_title('Speedup Relative to SCF')
    ax2.legend()
    ax2.grid(True)
    ...
    # Accuracy
    for name in ["OPT3", "Smart OPT3", "FBP"]:
        ax3.plot(water_sizes, results[name]['errors'], 'o-', label=name)
    ax3.set_xlabel('Number of Waters')
    ax3.set_ylabel('Error (%)')
    ax3.set_title('Energy Error vs Reference')
    ax3.legend()
    ax3.grid(True)
    ax3.set_yscale('log')
    
    plt.tight_layout()
    plt.savefig('/home/zhaomt/gcmc/test107/pygcmc_dev/tmp/fbp_performance.png', dpi=150)
    plt.close()
