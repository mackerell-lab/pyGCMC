...
void createWaterMolecule(vector<model::MCAtom>& atoms, double x, double y, double z) {
    model::MCAtom oxygen;
    oxygen.x = x;
    oxygen.y = y;
    oxygen.z = z;
...
int main() {
    ...
    // Parameters
    const double boxSize = 3.0;  // nm, can increase to 10-20
    ...
    vector<model::MCAtom> atoms;
    vector<model::MCResidue> residues;
    ...
    for (int ix = 0; ix < nPerDim; ix++) {
        for (int iy = 0; iy < nPerDim; iy++) {
            for (int iz = 0; iz < nPerDim; iz++) {
                ...
                // Create water
                int atomStart = atoms.size();
                createWaterMolecule(atoms, x, y, z);
                
                // Create residue
                model::MCResidue res;
                res.atomStart = atomStart;
                res.atomCount = 3;
                res.active = true;
                res.fixed = false;
                res.type = 0;
    ...
    state.atoms = atoms;
    state.activeAtomCount = atoms.size();
    state.residues = residues;
    state.activeResidueCount = residues.size();
    
    // Set force field - TIP3P
    model::MCForceField ff;
    ff.numTotalTypes = 2;
    ff.numMovementTypes = 2;
    
    // LJ parameters
    ff.ljSigma = {0.315, 0.0, 0.0, 0.0};  // O-O, O-H, H-O, H-H
    ff.ljEps = {0.636, 0.0, 0.0, 0.0};    // kJ/mol
    state.forcefield = ff;
    ...
    // Create simulation runner for energy calculations
    simulation::SimulationRunner runner(state);
    
    // Calculate initial energy
    auto start = high_resolution_clock::now();
    runner.computeSystemEnergyCutoff(state);
    ...
    double avgEnergy = 0, minEnergy = energies[0], maxEnergy = energies[0];
    double avgTime = 0, minTime = times[0], maxTime = times[0];
    
    for (size_t i = 0; i < energies.size(); i++) {
        avgEnergy += energies[i];
        avgTime += times[i];
        minEnergy = min(minEnergy, energies[i]);
        maxEnergy = max(maxEnergy, energies[i]);
    ...
    double stdEnergy = 0, stdTime = 0;
    for (size_t i = 0; i < energies.size(); i++) {
        stdEnergy += (energies[i] - avgEnergy) * (energies[i] - avgEnergy);
        stdTime += (times[i] - avgTime) * (times[i] - avgTime);
    }
    stdEnergy = sqrt(stdEnergy / energies.size());
    stdTime = sqrt(stdTime / times.size());
    
    cout << "\n=== Performance Summary ===\n";
    cout << "Total simulation time: " << totalTime << " seconds\n";
    cout << "Average time per step: " << avgTime << " ± " << stdTime << " ms\n";
    cout << "Min/Max time per step: " << minTime << " / " << maxTime << " ms\n";
    cout << "Steps per second: " << nSteps / totalTime << "\n";
    
    cout << "\n=== Energy Statistics ===\n";
    cout << "Average energy: " << avgEnergy << " ± " << stdEnergy << " kJ/mol\n";
    cout << "Min/Max energy: " << minEnergy << " / " << maxEnergy << " kJ/mol\n";
