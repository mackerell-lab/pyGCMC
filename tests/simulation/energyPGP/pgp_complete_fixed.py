...
def test_pgp_complete_movement_energy():
    ...
    elec_pgp_init, vdw_pgp_init, total_pgp_init = pgp_wrapper.computeSystemEnergyPGPComplete(state)
    ...
    elec_pgp_final, vdw_pgp_final, total_pgp_final = pgp_wrapper.computeSystemEnergyPGPComplete(state)
...
def test_pgp_complete_multi_atom_residue():
    ...
    pgp_wrapper.elec_pgp, vdw_pgp, total_pgp = pgp_wrapper.computeSystemEnergyPGPComplete(state)
...
def test_pgp_complete_direct_movement_test():
    ...
    elec_before, vdw_before, total_before = pgp_wrapper.computeSystemEnergyPGPComplete(state)
    
    # Move one particle slightly
    state.atoms[2].x += 0.1
    
    # Get system energy after movement  
    elec_after, vdw_after, total_after = pgp_wrapper.computeSystemEnergyPGPComplete(state)
    ...
    elec_pgp_mv, vdw_pgp_mv, pgp_dict = pgp_wrapper.computeMovementEnergyPGPComplete(state)
