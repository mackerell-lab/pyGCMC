# tests/simulation/movement/test_multi_insertion_robustness.py
"""Robustness and regression tests for multi-insertion CBMC."""
...
def make_test_state(box_size=3.0, cutoff=1.2):
    """Helper to create a test state."""
    state = pygcmc.MCState()
    state.info.box = np.array([box_size, box_size, box_size])  # nm
    state.info.cutoff = cutoff  # nm
    state.info.beta = 1.0 / 2.479  # kT @ 298K
    
    # Force field for water
    state.forcefield.numTotalTypes = 2  # O, H
    state.forcefield.numMovementTypes = 2
    state.forcefield.ljEps = [0.5, 0.45, 0.45, 0.2]  # O-O, O-H, H-O, H-H
    state.forcefield.ljSigma = [0.3, 0.25, 0.25, 0.2]  # nm
    
    return state


class TestMultiInsertionRobustness:
    """Test boundary conditions, monotonicity, and consistency."""
    
    @pytest.fixture
    def basic_system(self):
        """Create a basic test system."""
        state = pygcmc.MCState()
        state.info.box = np.array([3.0, 3.0, 3.0])  # nm
        state.info.cutoff = 1.2  # nm
        state.info.beta = 1.0 / 2.479  # kT @ 298K
        
        # Force field for water (matching test_multi_insertion_improved.py)
        state.forcefield.numTotalTypes = 2  # O, H
        state.forcefield.numMovementTypes = 2
        state.forcefield.ljEps = [0.5, 0.45, 0.45, 0.2]  # O-O, O-H, H-O, H-H
        state.forcefield.ljSigma = [0.3, 0.25, 0.25, 0.2]  # nm
    ...
    def test_acceptance_rate_monotonicity_chemical_potential(self, basic_system):
        """Test acceptance rate increases with chemical potential."""
        state, params = basic_system
    ...
    def test_acceptance_rate_monotonicity_volume(self, basic_system):
    ...
    def test_energy_consistency_single_vs_system(self, basic_system):
    ...
    def test_region_independence_boundary(self, basic_system):
        ...
        # Should still work but select only 1 region
    ...
    def test_deterministic_config_selection(self, basic_system):
    ...
    def test_fallback_path_unit_consistency(self, basic_system):
    ...
    def test_energy_range_parameterization(self, basic_system):
    ...
    def test_openmp_determinism(self, basic_system):
