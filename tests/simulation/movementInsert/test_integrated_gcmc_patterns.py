...
def test_convergence_monitoring():
    ...
    assert len(tracker.n_molecules_history) > 0, "No history recorded"
    
    # 如果收敛，验证最后的状态稳定
    if converged:
        last_values = tracker.n_molecules_history[-20:]
        std_dev = np.std(last_values)
        mean_val = np.mean(last_values)
        if mean_val > 0:
            cv = std_dev / mean_val  # 变异系数
            assert cv < 0.3, f"System not stable after convergence: CV={cv}"
...
def test_realistic_waterbox_setup():
    ...
    # 验证接受率在合理范围
    insert_rate = tracker.get_acceptance_rate('insert')
    if tracker.moves['insert']['accepted'] + tracker.moves['insert']['rejected'] > 10:
