# tests/simulation/energyOpenmm/method_cutoff_nonperiodic.py

import pytest
from .method_helpers import *

def test_compare_nonbonded_cutoff_nonperiodic():
    """
    Compare CutoffNonPeriodic nonbonded method between CustomNonbondedForce and NonbondedForce.
    
    Tests CutoffNonPeriodic calculation method:
    1. Cutoff with reaction field:
    ...
    - Ensures differences are within method-specific tolerance
    """
    # Create test system
    system, topology, positions = create_test_system()
    
    # Test CutoffNonPeriodic method
    nb_method = NonbondedForce.CutoffNonPeriodic
    custom_method = CustomNonbondedForce.CutoffNonPeriodic
    method_name = "CutoffNonPeriodic"
    
    # Define tolerance for this method
    rel_tol = 5e-4  # Allow 0.05% error when using cutoff
    ...
    # Energy expression for CutoffNonPeriodic method with switching function
    energy_expression = """
    step(cutoff - r) * (
    ...
    # Apply self-energy correction for cutoff method
    ...
    # Use tolerance for verification
