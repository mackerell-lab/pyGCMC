# tests/simulation/movementCPP/proposal_modes_improved_funcs.py
"""Improved proposal mode test functions with physics validation."""

import pytest
import pygcmc
import numpy as np
import math
from .proposal_modes_fixtures import clean_system


def test_uniform_mode_statistical_uniformity(clean_system):
...
def test_cavity_mode_with_geometry_validation(clean_system):
...
def test_mode_acceptance_rate_comparison(clean_system):
...
def test_density_effect_on_cavities():
...
def test_mode_clamping_behavior():
...
def test_statistics_aggregation(clean_system):
