...
// Thread-safe FFT weights management
class FFTWeightsManager {
private:
...
// Global instance with proper lifetime management
static std::unique_ptr<FFTWeightsManager> weights_manager;
static std::mutex manager_mutex;

// Get or create weights manager
static FFTWeightsManager& getWeightsManager() {
    std::lock_guard<std::mutex> lock(manager_mutex);
    if (!weights_manager) {
        weights_manager = std::make_unique<FFTWeightsManager>();
    }
...
void clearFFTWeights() {
    std::lock_guard<std::mutex> lock(manager_mutex);
    if (weights_manager) {
        weights_manager->clear();
        // Don't reset the unique_ptr here, keep the manager alive
        // This prevents issues with dangling pointers
    }
}
