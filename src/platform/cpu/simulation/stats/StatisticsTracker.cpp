...
void StatisticsTracker::recordMove(const std::string& moveType, 
...
void StatisticsTracker::recordEnergy(double energy) {
...
void StatisticsTracker::recordFragmentCount(const std::string& name, int count) {
    fragmentStats_[name].currentCount = count;
}

void StatisticsTracker::recordStepTime(double seconds) {
    totalTime_ += seconds;
    stepTimes_.push_back(seconds);
}

void StatisticsTracker::updateStatistics() {
...
void StatisticsTracker::updateFragmentDensity(const std::string& name, double density) {
    fragmentStats_[name].density = density;
}

StatisticsTracker::MoveStats 
StatisticsTracker::getMoveStats(const std::string& moveType) const {
    auto it = moveStats_.find(moveType);
    return it != moveStats_.end() ? it->second : MoveStats();
}

StatisticsTracker::FragmentStats 
StatisticsTracker::getFragmentStats(const std::string& name) const {
...
double StatisticsTracker::getEnergyStdDev() const {
    return calculateStdDev(energySum_, energySumSquared_, energySamples_);
}

double StatisticsTracker::getTotalAcceptanceRate() const {
    return totalSteps_ > 0 ? 
        static_cast<double>(totalAccepted_) / totalSteps_ : 0.0;
}

double StatisticsTracker::getStepsPerSecond() const {
    return totalTime_ > 0 ? totalSteps_ / totalTime_ : 0.0;
}

void StatisticsTracker::printSummary(int step) const {
...
void StatisticsTracker::printDetailedStats() const {
...
std::string StatisticsTracker::formatStatistics() const {
    std::stringstream ss;
...
void StatisticsTracker::updateAverages() {
    // Update timing
...
double StatisticsTracker::calculateStdDev(double sum, double sumSquared, int n) const {
    if (n <= 1) return 0.0;
