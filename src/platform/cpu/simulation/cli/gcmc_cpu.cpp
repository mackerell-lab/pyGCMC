...
struct Options {
    ...
    bool strict_inp_keys = false;
    bool strict_inp = false;
};
...
void print_usage(const char* prog) {
    ...
    std::cout << "  --strict-inp-keys          Fail if INP contains unknown/ignored keys\n";
    std::cout << "  --strict-inp               Fail if INP has unknown/ignored keys OR heuristic warnings\n";
}
...
bool parse_args(int argc, char** argv, Options& opt) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--inp") {
            ...
            opt.store_prob = true;
        } else if (a == "--strict-inp-keys") {
            opt.strict_inp_keys = true;
        } else if (a == "--strict-inp") {
            opt.strict_inp = true;
...
int main(int argc, char** argv) {
    ...
    cfg.strictInpKeys = opt.strict_inp_keys || opt.strict_inp;
    cfg.strictInpWarnings = opt.strict_inp;
