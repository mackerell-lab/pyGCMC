...
public:
    ...
    std::string to_string() const {
        std::stringstream ss;
        ss << "ForceField: " << get_num_atom_types() << " atom types, "
           << get_num_bond_types() << " bond types, "
           << get_num_angle_types() << " angle types, "
           << get_num_dihedral_types() << " dihedral types, "
           << get_num_improper_types() << " improper types";
        if (get_num_nbfix() > 0) {
            ss << ", " << get_num_nbfix() << " NBFIX entries";
        }
        return ss.str();
    }
...
// Backward compatibility: provide the ForceField classes in the model namespace
